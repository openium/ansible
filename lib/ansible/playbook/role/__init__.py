...
class Role(Base, Become, Conditional, Taggable):
    ...
    def _load_role_data(self, role_include, parent_role=None):
        ...
        if not any((metadata, task_data, handler_data)) and len(os.listdir(self._role_path)) == 0:
            raise AnsibleParserError("Role %s does not contain any data" % self._role_name)
